...
/*   Updated: 2019/08/06 21:31:29 by myuliia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	editor_player_events(t_doom *doom)
{
	...
	while (SDL_PollEvent(&doom->ev) && doom->game.quit != 1)
	{
		...
		if (doom->ev.type == SDL_KEYDOWN)
		{
			...
			if (doom->ev.key.keysym.sym == SDLK_PAGEUP || doom->ev.key.keysym.sym == SDLK_PAGEDOWN)
			{
				if (doom->ev.key.keysym.sym == SDLK_PAGEDOWN && doom->editor.fl_or_ceil > 1 &&  (doom->editor.fl_or_ceil--))
					info_f_c_w_s(doom, 1);
				else if (doom->ev.key.keysym.sym == SDLK_PAGEUP && doom->editor.fl_or_ceil < PAINTINGS && (doom->editor.fl_or_ceil++))
					info_f_c_w_s(doom, 1);	
			}
