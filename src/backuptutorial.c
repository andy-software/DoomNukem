	...
	*head = (struct item) { player.sector, 0, W-1 };
	...
	do {
		...
		for(unsigned s = 0; s < sect->npoints; ++s)
		{
			...
			float tx1 = vx1 * psin - vy1 * pcos,  ty1 = vx1 * pcos + vy1 * psin;
			float tx2 = vx2 * psin - vy2 * pcos,  ty2 = vx2 * pcos + vy2 * psin;
			/* Is the wall at least partially in front of the player? */
			
			if(ty1 <= 0 && ty2 <= 0) continue;
			/* If it's partially behind the player, clip it against player's view frustrum */
			if(ty1 <= 0 || ty2 <= 0)
			{
				float neary = 1e-3f, fary = 1.f, nearside = 1e-4f, farside = 5.f;
				// Find an intersection between the wall and the approximate edges of player's view
				struct xy i1 = Intersect(tx1,ty1,tx2,ty2, -nearside,neary, -farside,fary);
				struct xy i2 = Intersect(tx1,ty1,tx2,ty2,  nearside,neary,  farside,fary);
				if(ty1 < 0)
				{
					if(i1.y > 0)
					{
			...
			float xscale1 = hfov / ty1, yscale1 = vfov / ty1;    int x1 = W/2 - (int)(tx1 * xscale1);
			float xscale2 = hfov / ty2, yscale2 = vfov / ty2;    int x2 = W/2 - (int)(tx2 * xscale2);
			...
			float nzceil=0, nzfloor=0;
			if(neighbor >= 0) // Is another sector showing through this portal?
			{
				nzceil  = sectors[neighbor].ceil  - player.where.z;
				nzfloor = sectors[neighbor].floor - player.where.z;
			}
			int z1a  = H/2 - (int)(Yaw(zceil, ty1) * yscale1),  z1b = H/2 - (int)(Yaw(zfloor, ty1) * yscale1);
			int z2a  = H/2 - (int)(Yaw(zceil, ty2) * yscale2),  z2b = H/2 - (int)(Yaw(zfloor, ty2) * yscale2);
			/* The same for the neighboring sector */
			int nz1a = H/2 - (int)(Yaw(nzceil, ty1) * yscale1), nz1b = H/2 - (int)(Yaw(nzfloor, ty1) * yscale1);
			int nz2a = H/2 - (int)(Yaw(nzceil, ty2) * yscale2), nz2b = H/2 - (int)(Yaw(nzfloor, ty2) * yscale2);
			...
			for(int x = beginx; x <= endx; ++x)
			{
				...
				int z = ((x - x1) * (ty2-ty1) / (x2-x1) + ty1) * 8;
				/* Acquire the Y coordinates for our ceiling & floor for this X coordinate. Clamp them. */
				int za = (x - x1) * (z2a-z1a) / (x2-x1) + z1a, cza = clamp(za, ytop[x],ybottom[x]); // top
				int zb = (x - x1) * (z2b-z1b) / (x2-x1) + z1b, czb = clamp(zb, ytop[x],ybottom[x]); // bottom
				
				/* Render ceiling: everything above this sector's ceiling height. */
				vline(x, ytop[x], cza-1, 0x111111 ,0x222222,0x111111);
				/* Render floor: everything below this sector's floor height. */
				vline(x, czb+1, ybottom[x], 0x0000FF,0x0000AA,0x0000FF);
				
				/* Is there another sector behind this edge? */
				if(neighbor >= 0)
				{
					/* Same for _their_ floor and ceiling */
					int nza = (x - x1) * (nz2a-nz1a) / (x2-x1) + nz1a, cnza = clamp(nza, ytop[x],ybottom[x]);
					int nzb = (x - x1) * (nz2b-nz1b) / (x2-x1) + nz1b, cnzb = clamp(nzb, ytop[x],ybottom[x]);
					/* If our ceiling is higher than their ceiling, render upper wall */
					unsigned r1 = 0x010101 * (255-z), r2 = 0x040007 * (31-z/8);
					vline(x, cza, cnza-1, 0, x==x1||x==x2 ? 0 : r1, 0); // Between our and their ceiling
					ytop[x] = clamp(max(cza, cnza), ytop[x], H-1);   // Shrink the remaining window below these ceilings
					/* If our floor is lower than their floor, render bottom wall */
					vline(x, cnzb+1, czb, 0, x==x1||x==x2 ? 0 : r2, 0); // Between their and our floor
					ybottom[x] = clamp(min(czb, cnzb), 0, ybottom[x]); // Shrink the remaining window above these floors
				}
				else
				{
					/* There's no neighbor. Render wall from top (cya = ceiling level) to bottom (cyb = floor level). */
					unsigned r = 0x010101 * (255-z);
					vline(x, cza, czb, 0, x==x1||x==x2 ? 0 : r, 0);
				}
			}
...
int main()
{
	...
	while (1)
	{
		//bzero(surface->pixels, surface->h * surface->w * 4);
		DrawScreen();
